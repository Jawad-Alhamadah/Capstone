...
#allowed origins
origins = [
    "http://localhost:3000","https://en.wikipedia.org"
]
#the response object
...
async def home(canvasImage: UploadFile = File(...)):
    ...
    for link in links:
        ...
        if link.get("title") is not None:
            ...
            wiki_link =link.get("title").lower()
            if dog_name in wiki_link  or  wiki_link  in dog_name :
...
#ap.add_argument( "--image", required=True,help="C:\\Users\\moon2\\Documents\\pyImage\\laptop-serial-number-sticker.jpg")
...
#print(breeds_dict[breed].replace('_'," "))    
def predict_breed_transfer(img_path):
    # load the image and return the predicted breed
    model_transfer.eval()
    target =torch.Tensor([5])
...
def run_app(img_path):
   ...
   # plt.imshow(image)
    ...
    return "error, we could not detect a human or a dog in the photo."
